...
| **Session memory** | Yes - a machine-local ledger keyed to a working-tree fingerprint: a failed run on unchanged code is declined, the same failure across edits and pass/fail on identical code are named, and a context pack lists a file it already served instead of repeating it | No | No | No | No | No | No | Hosted chat history | No |
